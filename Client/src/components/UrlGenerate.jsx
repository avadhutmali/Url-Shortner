...
function UrlGenerate() {
    ...
    return (
        <div className='h-screen w-full flex items-center justify-center bg-gray-900'>
            <div
                className="absolute top-2 right-2 bg-red-600 h-10 w-20 text-center p-2 rounded-xl text-white cursor-pointer"
                onClick={handleLogout}
            >
                Logout
            </div>
            <div
                className="absolute top-2 right-32 bg-blue-600 h-10 w-28 text-center p-2 rounded-xl text-white cursor-pointer"
            ...
            <div className="h-[60vh] w-[70vw] bg-gray-800 rounded-xl flex flex-col gap-4">
                ...
                <div className="flex gap-2 mt-20">
                    <input
                        className='pl-8 text-xl h-20 ml-4 w-full bg-white rounded-xl'
                        type="text"
                        value={redireUrl}
                        onChange={(e) => { setRedireUrl(e.target.value) }}
                    />
                    <button
                        className='h-20 w-36 p-2 mr-4 font-semibold text-white text-xl bg-blue-600 rounded-xl'
